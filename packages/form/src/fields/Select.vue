...
import { ElSelect } from '@edoms/design';
...
const edomsSelect = ref<InstanceType<typeof ElSelect>>();
...
props.config.remote &&
  onMounted(async () => {
    await nextTick();
    edomsSelect.value?.scrollbarWrap?.addEventListener('scroll', async (e: Event) => {
...
const visibleHandler = async (visible: boolean) => {
  ...
  if (query.value && edomsSelect.value) {
    edomsSelect.value.setQuery(query.value);
    edomsSelect.value.setPreviousQuery(query.value);
    edomsSelect.value.setSelectedLabel(query.value);
