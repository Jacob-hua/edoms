<template>
  <div ref="mTable" class="m-fields-table" :class="{ 'm-fields-table-item-extra': config.itemExtra }">
    ...
    <div class="el-form-item__content">
      <ElTooltip content="拖拽可排序" placement="left-start" :disabled="config.dropSort !== true">
        <ElTable
        ...
        >
          <ElTableColumn v-if="config.itemExtra" :fixed="'left'" width="30" type="expand">
            <template #default="scope">
              <span class="m-form-tip" v-html="itemExtra(config.itemExtra, scope.$index)"></span>
            </template>
          </ElTableColumn>

          <ElTableColumn label="操作" width="55" align="center" :fixed="config.fixed === false ? undefined : 'left'">
            <template #default="scope">
              <ElIcon
                v-show="showDelete(scope.$index + 1 + pagecontext * pagesize - 1)"
                class="edoms-table-delete-icon"
          ...
          <ElTableColumn v-if="sort && model[modelName] && model[modelName].length > 1" label="排序" width="60">
            <template #default="scope">
              <ElTooltip
          ...
          <ElTableColumn v-if="showIndex && config.showIndex" width="60" label="序号">
      ...
      >
        <ElButton size="small" type="success" plain>导入EXCEL</ElButton> </ElUpload
      >&nbsp;
      <ElButton v-if="importable" size="small" type="warning" plain @click="clearHandler">清空</ElButton>
    </div>
...
import { ElButton, ElIcon, elMessage, ElPagination, ElTable, ElTableColumn, ElTooltip, ElUpload } from '@edoms/design';
import { asyncLoadJs, sleep } from '@edoms/utils';
...
const edomsTable = ref<InstanceType<typeof ElTable>>();
...
const rowDrop = () => {
  const tableEl = edomsTable.value?.$el;
...
const selectHandle = (selection: any, row: any) => {
  if (typeof props.config.selection === 'string' && props.config.selection === 'single') {
    edomsTable.value?.clearSelection();
    edomsTable.value?.toggleRowSelection(row, true);
  }
...
const toggleRowSelection = (row: any, selected: boolean) => {
  edomsTable.value?.toggleRowSelection.call(edomsTable.value, row, selected);
};
