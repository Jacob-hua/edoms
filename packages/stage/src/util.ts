...
export const removeSelectedClassName = (doc: Document) => {
  ...
  if (oldEl) {
    dom.removeClassName(oldEl, SELECTED_CLASS);
    if (oldEl.parentNode) dom.removeClassName(oldEl.parentNode as Element, `${SELECTED_CLASS}-parent`);
    doc.querySelectorAll(`.${SELECTED_CLASS}-parents`).forEach((item) => {
      dom.removeClassName(item, `${SELECTED_CLASS}-parents`);
    });
  }
};
