...
import type { EdId } from '@edoms/meta-model';
...
export default class StageCore extends EventBus {
  ...
  constructor(config: StageCoreConfig) {
    ...
    this.mask.on('changeGuides', (data: GuidesEventData) => {
    ...
    this.mask.on('clearHighlight', async () => {
      this.highlightLayer.clearHighlight();
    });
    this.mask.on('beforeMultiSelect', async (event: MouseEvent) => {
      const el = await this.setElementFromPoint(event);
      if (!el) return;
      ...
      this.fire('multiSelect', this.selectedDomList);
    });

    // 要先触发select，在触发update
    this.dr.on('update', (data: UpdateEventData) => {
      setTimeout(() => this.fire('update', data));
    });
    this.dr.on('sort', (data: UpdateEventData) => {
      setTimeout(() => this.fire('sort', data));
    });
   ...
   */
  public async select(idOrEl: EdId | HTMLElement, event?: MouseEvent): Promise<void> {
  ...
  private async getTargetElement(idOrEl: EdId | HTMLElement): Promise<HTMLElement> {
