<template>
  <div class="table-list">
    <div v-for="(tbItm, idx) in state.tableDataList" :key="idx" class="table-item-list">
      ...
      <div class="operation">
        <span v-show="!tbItm.isCol" class="elft" @click="handlerToCol(true, idx)">下</span>
        <span v-show="tbItm.isCol" class="elft" @click="handlerToCol(false, idx)">上</span>
...
const getValue = computed(() => (_itm: any) => {
  const value = allWarningList.filter((itm: any) => itm.warningCode === _itm)[0]?.typeName;
  return value;
});
