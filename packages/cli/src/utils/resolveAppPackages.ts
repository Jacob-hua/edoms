...
const parseEntry = function ({ ast, package: module, indexPath }: ParseEntryOption) {
  ...
  [, config] = config.match(reg) || [undefined, config];
  [, value] = value.match(reg) || [undefined, value];
  [, component] = component.match(reg) || [undefined, component];
  [, event] = event.match(reg) || [undefined, event];
