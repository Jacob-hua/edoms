...
export const scripts = (defaultAppConfig: UserConfig) => {
  const entry = async (): Promise<void> => {
    ...
    const userConfigPath = [
      path.resolve(defaultAppConfig.source, 'edoms.config.ts'),
      path.resolve(defaultAppConfig.source, 'edoms.config.js'),
      path.resolve(defaultAppConfig.source, 'edoms.config.cjs'),
